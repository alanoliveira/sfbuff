...
RSpec.describe Matchup::Scorable do
  let(:matchup) { Matchup.where(home_challenger: { short_id: 1234567890 }) }
  ...
  describe "#score" do
    ...
    context "when selecting additional data" do
      let(:score) do
        matchup
          .group(away_challenger: [ :short_id ])
          .select(away_challenger: [ :short_id ])
          .score
      end
