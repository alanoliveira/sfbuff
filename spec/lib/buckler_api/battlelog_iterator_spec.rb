...
RSpec.describe BucklerApi::BattlelogIterator do
  subject(:replays) { described_class.new(client, short_id) }

  let(:client) { instance_double BucklerApi::Client }
  let(:short_id) { "12345678" }

  def mock_request(page, result)
    allow(client).to receive(:battlelog).with(short_id, page).and_return(result)
  end
