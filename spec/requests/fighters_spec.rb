  ...
  describe "POST /synchronize" do
    ...
    context "when the fighter is already synchronized" do
      it "returns http see_other" do
        ...
        post "/fighters/111222333/synchronize", headers: { accept: "text/vnd.turbo-stream.html", referer: fighters_path(111222333) }

        expect(response).to have_http_status(:see_other).and redirect_to(fighters_path(111222333))
      end
    end
  end
end
